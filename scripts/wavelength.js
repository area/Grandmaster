...
// 	})
	...
	robot.hear(/!wavelength <@!(.*)>/, async(res) => {
		...
		board = await makeBoard(gaugeAngle, armAngle, scale)
		user = robot.client.users.get(res.match[1])
	...
	robot.hear(/!wavelength ([+-][0-9]*)/, async (res) => {
		// if (isPrivateSlackMessage(res)) { return }
		console.log(res)

		delta = -parseFloat(res.match[1]);
		if (armAngle <= 80 && armAngle + delta > 80){ armAngle = 80 }
		else if (armAngle >= 280 && armAngle + delta < 280){ armAngle = 280 }
		else if (!validAngle(armAngle + delta)){ return }
		else {
			armAngle = (armAngle + delta + 360) % 360
		}
		board = await makeBoard(null, armAngle, scale)
		channel = await robot.client.channels.get(res.message.room)
	...
	robot.hear(/!wavelength reveal/, async (res) => {
		if (isPrivateSlackMessage(res)) { return }
	...
	async function makeBoard(gaugeAngle, armAngle, scale){
		// Load the base image
		...
		if (gaugeAngle){
			...
			gaugeanglerad = gaugeAngle * 3.14159 / 180
			...
			if (gaugeAngle >= 0 && gaugeAngle <= 80){
				...
				x = R * Math.sin(gaugeanglerad) + c * Math.cos(gaugeanglerad) - c
				y = R - R*Math.cos(gaugeanglerad) - c * Math.sin(gaugeanglerad)
				...
				x = R * Math.sin(2*3.14159 - gaugeanglerad) + c * Math.cos(2*3.14159 -gaugeanglerad) - c
				y = R - R*Math.cos(gaugeanglerad) + c * Math.sin(gaugeanglerad)
		...
		if (armAngle){
			// add the arm
			const arm = await Jimp.read('./assets/wavelength/arm.jpg')
			armangle = parseFloat(armAngle)
